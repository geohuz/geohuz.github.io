...
import React, { useMemo, useState, useLayoutEffect, useEffect, useRef, useContext, useCallback } from 'react';
...
export const Text = ({ text, fontSize, textAlign, ...props }) => {
  ...
  function updateFromRichEditor(value) {
    console.log("richeditor value: ", value)
    slateEditor.children = deserialize(value)
  ...
  return (
    ...
    <Observer>
      ...
      >
        ...
        <PopupEditor open={appStore.richTextMode===true} initialValue={initialValue} update={updateFromRichEditor}/>
      </div>
      }
      </Observer>
      </Resizer>
...
function PopupEditor({open, initialValue, update}) {
  const handleClose = () => {
    let text = ref.current.saveChange()
    update(text)
